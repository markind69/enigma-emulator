...
class TestEnigma(unittest.TestCase):
    ...
    def testRotorIds(self):
        ...
        assert e.rotor1.id == 'I', "Failed to assign rotor correctly"

    def testRotorPositions(self):
        e = Enigma(self.createRotorConfig())
        assert e.rotor1.position == 0, "Failed to assign rotor position correctly"

    def testAdvanceRightRotor(self):
        e = Enigma(self.createRotorConfig())
        pos1 = e.rotor1.position
        e.encode('a')
        assert e.rotor1.position == pos1 + 1, "Failed to advance right rotor on encode"
    ...
    def testAdvanceMiddleRotor(self):
        ...
        e.encode('a')
        assert e.rotor2.position == pos2 + 1, "Failed to advance middle rotor on encode"
    ...
    def testAdvanceLeftRotor(self):
        ...
        e.encode('a')
        assert e.rotor3.position == pos3 + 1, "Failed to advance left rotor on encode"
    ...
    #
    # Note that "HELLO" will not necessarily encode to "AMNCZ" with these same
    # settings in other emulators, depending on the wiring of the rotors and other
    # variations. While this matches a few different emulators out there and fits
    # to my very imperfect understanding of how the Enigma worked, I make no claim
    # that the results are historically accurate.
    def testString(self):
        e = Enigma(self.createRotorConfig())
        msg = e.encode('HELLO')
